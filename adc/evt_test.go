...
// ── REQ-ADC-011: TC18 §13.5 Table 30 / §12.9.1 evt handling ──
//
// Table 30 puts ADC in the row it shares with ADC, PWM_IN, I²C, LIN, CAN,
...
// endpoint type (REQ-ADC-011).
func TestEVTClass_IsTable30ConfigOnlyRow(t *testing.T) {
	if adc.EVTClass != acf.EVTClassConfigOnly {
		t.Errorf("adc.EVTClass = %v, want %v", adc.EVTClass, acf.EVTClassConfigOnly)
	}
}
...
// reaches the interface on the way (REQ-ADC-011).
func TestEVT_ReservedSelectorsRejected(t *testing.T) {
	ep, root := newDeclaredEndpoint(t)
	if err := ep.Configure(root, adc.Config{Enabled: true, ResolutionBits: 8, SampleCount: 1, Combine: adc.CombineReplace, TriggerMode: adc.TriggerModeExternal}); err != nil {
		t.Fatalf("Configure: %v", err)
	}
	tr := &sequenceTransport{samples: []uint16{42}}
	ep.SetTransport(tr)

	for sel := acf.EVTSelector(1); sel <= 6; sel++ {
		if _, err := ep.HandleRequest(root, evtReq(sel, acf.FlagRead, []byte{0x00})); !errors.Is(err, acf.ErrEVTReserved) {
			t.Errorf("evt[2:0]=%03b: err = %v, want acf.ErrEVTReserved", sel, err)
		}
	}
	if tr.i != 0 {
		t.Errorf("reserved requests reached the interface (tr.i = %v), want none", tr.i)
	}
}
...
// response shall be sent with the error code = UNSUPPORTED_CMD" (REQ-ADC-011).
func TestEVT_NonZeroWithoutPayloadRejected(t *testing.T) {
	...
	for sel := acf.EVTSelector(1); sel <= 7; sel++ {
		if _, err := ep.HandleRequest(root, evtReq(sel, acf.FlagRead, nil)); !errors.Is(err, acf.ErrEVTMissingPayload) {
...
func TestEVT_ConfigChangeIsNotPresentedAtInterface(t *testing.T) {
	...
	if tr.i != 0 {
		t.Errorf("configuration payload reached the interface (tr.i = %v), want none", tr.i)
	}

	// The configuration really was adopted: the endpoint now reports itself
	// unconfigured for ordinary traffic.
	if _, err := ep.HandleRequest(root, evtReq(acf.EVTSelector0, acf.FlagRead, []byte{0x00})); !errors.Is(err, adc.ErrChannelNotConfigured) {
		t.Errorf("after configuration write: err = %v, want %v", err, adc.ErrChannelNotConfigured)
	}
}
