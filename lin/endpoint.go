...
// Transport performs the endpoint's actual byte exchange on the LIN bus:
// given the raw bytes to place on the wire, it returns the raw bytes
// received back, if any. This package has no real LIN bus of its own to
...
// driver (one that, per doc.go's Scope section, owns any PID/checksum/
// schedule-table framing itself). An endpoint with no Transport set defaults
// to a loopback echo (see Endpoint.transfer).
type Transport interface {
	Transfer(tx []byte) (rx []byte, err error)
}

// TriggerEvent records one transaction-complete signal a LIN endpoint
// queued, carrying the number of bytes received back over the bus.
type TriggerEvent struct {
	ByteCount int
}

// Endpoint is one declared LIN endpoint layered on top of a server.Server:
...
// NewEndpoint returns a LIN Endpoint for the already-declared endpoint addr
