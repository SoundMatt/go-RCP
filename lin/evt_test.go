...
// ── REQ-LINEP-009: TC18 §13.5 Table 30 / §12.9.1 evt handling ──
//
// Table 30 puts LIN in the row it shares with ADC, PWM_IN, I²C, LIN, CAN,
...
// endpoint type (REQ-LINEP-009).
func TestEVTClass_IsTable30ConfigOnlyRow(t *testing.T) {
	if lin.EVTClass != acf.EVTClassConfigOnly {
		t.Errorf("lin.EVTClass = %v, want %v", lin.EVTClass, acf.EVTClassConfigOnly)
	}
}
...
// reaches the interface on the way (REQ-LINEP-009).
func TestEVT_ReservedSelectorsRejected(t *testing.T) {
	...
	for sel := acf.EVTSelector(1); sel <= 6; sel++ {
		if _, err := ep.HandleRequest(root, evtReq(sel, acf.FlagWrite, []byte{0x3C, 0x01})); !errors.Is(err, acf.ErrEVTReserved) {
...
// response shall be sent with the error code = UNSUPPORTED_CMD" (REQ-LINEP-009).
...
func TestEVT_ConfigChangeIsNotPresentedAtInterface(t *testing.T) {
	...
	if _, err := ep.HandleRequest(root, evtReq(acf.EVTSelector0, acf.FlagWrite, []byte{0x3C, 0x01})); !errors.Is(err, lin.ErrBusNotConfigured) {
		t.Errorf("after configuration write: err = %v, want %v", err, lin.ErrBusNotConfigured)
	}
}
