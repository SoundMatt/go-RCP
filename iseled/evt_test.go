...
// ── REQ-ISELED-010: TC18 §13.5 Table 30 / §12.9.1 evt handling ──
//
// Table 30 puts ISELED in the row it shares with ADC, PWM_IN, I²C, LIN, CAN,
...
// endpoint type (REQ-ISELED-010).
func TestEVTClass_IsTable30ConfigOnlyRow(t *testing.T) {
	if iseled.EVTClass != acf.EVTClassConfigOnly {
		t.Errorf("iseled.EVTClass = %v, want %v", iseled.EVTClass, acf.EVTClassConfigOnly)
	}
}
...
// reaches the interface on the way (REQ-ISELED-010).
func TestEVT_ReservedSelectorsRejected(t *testing.T) {
	...
	for sel := acf.EVTSelector(1); sel <= 6; sel++ {
		if _, err := ep.HandleRequest(root, evtReq(sel, acf.FlagWrite, iseled.EncodeCommand(iseled.Command{Address: 0, Data: []byte{0x01}}))); !errors.Is(err, acf.ErrEVTReserved) {
...
// response shall be sent with the error code = UNSUPPORTED_CMD" (REQ-ISELED-010).
...
func TestEVT_ConfigChangeIsNotPresentedAtInterface(t *testing.T) {
	...
	if _, err := ep.HandleRequest(root, evtReq(acf.EVTSelector0, acf.FlagWrite, iseled.EncodeCommand(iseled.Command{Address: 0, Data: []byte{0x01}}))); !errors.Is(err, iseled.ErrChainNotConfigured) {
		t.Errorf("after configuration write: err = %v, want %v", err, iseled.ErrChainNotConfigured)
	}
}
