...
// ── REQ-UART-011: TC18 §13.5 Table 30 / §12.9.1 evt handling ──
//
// Table 30 puts UART in the row it shares with ADC, PWM_IN, I²C, LIN, CAN,
...
// endpoint type (REQ-UART-011).
func TestEVTClass_IsTable30ConfigOnlyRow(t *testing.T) {
	if uart.EVTClass != acf.EVTClassConfigOnly {
		t.Errorf("uart.EVTClass = %v, want %v", uart.EVTClass, acf.EVTClassConfigOnly)
	}
}
...
// reaches the interface on the way (REQ-UART-011).
func TestEVT_ReservedSelectorsRejected(t *testing.T) {
	ep, root := newDeclaredEndpoint(t)
	if err := ep.Configure(root, uart.Config{Enabled: true, BaudRate: 115200, DataBits: 8, Parity: uart.ParityNone, StopBits: uart.StopBitsOne}); err != nil {
	...
	for sel := acf.EVTSelector(1); sel <= 6; sel++ {
		if _, err := ep.HandleRequest(root, evtReq(sel, acf.FlagWrite, uart.EncodeWriteRequest([]byte{0x01, 0x02}))); !errors.Is(err, acf.ErrEVTReserved) {
			t.Errorf("evt[2:0]=%03b: err = %v, want acf.ErrEVTReserved", sel, err)
		}
	}
	if tr.calls != 0 {
		t.Errorf("reserved requests reached the interface (tr.calls = %v), want none", tr.calls)
	}
}
...
// response shall be sent with the error code = UNSUPPORTED_CMD" (REQ-UART-011).
...
func TestEVT_ConfigChangeIsNotPresentedAtInterface(t *testing.T) {
	...
	if tr.calls != 0 {
		t.Errorf("configuration payload reached the interface (tr.calls = %v), want none", tr.calls)
	}

	// The configuration really was adopted: the endpoint now reports itself
	// unconfigured for ordinary traffic.
	if _, err := ep.HandleRequest(root, evtReq(acf.EVTSelector0, acf.FlagWrite, uart.EncodeWriteRequest([]byte{0x01, 0x02}))); !errors.Is(err, uart.ErrUARTNotConfigured) {
		t.Errorf("after configuration write: err = %v, want %v", err, uart.ErrUARTNotConfigured)
	}
}
