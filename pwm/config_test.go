...
// default waveform must not have ActiveMicros exceed PeriodMicros
// (REQ-PWM-001).
func TestConfig_Validate(t *testing.T) {
	tests := []struct {
		...
		{"output active exceeds period", pwm.Config{Enabled: true, Role: pwm.RoleOutput, DefaultPeriodMicros: 100, DefaultActiveMicros: 200}, pwm.ErrActiveExceedsPeriod},
		{"output valid", pwm.Config{Enabled: true, Role: pwm.RoleOutput, DefaultPeriodMicros: 1000, DefaultActiveMicros: 500}, nil},
		{"input ignores default fields", pwm.Config{Enabled: true, Role: pwm.RoleInput, DefaultPeriodMicros: 1, DefaultActiveMicros: 1000}, nil},
	}
...
func TestWaveformRoundTrip(t *testing.T) {
	b := pwm.EncodeWaveform(20000, 1500)
	period, active, err := pwm.DecodeWaveform(b)
	if err != nil {
		t.Fatalf("DecodeWaveform: %v", err)
	}
	if period != 20000 || active != 1500 {
		t.Errorf("DecodeWaveform round-trip = (%d, %d), want (20000, 1500)", period, active)
	}
